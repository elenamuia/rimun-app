...
class TodayScreen extends StatefulWidget {
  ...
  final ScheduleService scheduleService; // per ora non lo usiamo, ma lo teniamo
...
class _TodayScreenState extends State<TodayScreen> {
  ...
  Future<List<DaySchedule>> _loadEventsFromCsv() async {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      // salta l'header
      if (i == 0 && line.toLowerCase().startsWith('day')) {
        continue;
      }

      // split base: day, start, end, description, location
      final parts = line.split(',');
      if (parts.length < 5) {
        // se una riga è malformata, la saltiamo
        continue;
      }
      ...
      // nel caso la description contenga virgole, uniamo tutto da index 3 in poi
    ...
    // Ordiniamo gli eventi all'interno di ogni giorno
    ...
    // Se vuoi ordinare i giorni per data, qui potresti farlo;
    // per ora manteniamo l'ordine del CSV.
